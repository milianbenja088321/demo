...
public class ReturnOnReleaseGrab : Photon.MonoBehaviour
{
    ...
    private void OnTriggerStay(Collider other)
    {
        foreach (SteamVR_TrackedController controller in steamvr_controller_list)
        {
            if (controller.gameObject == other.gameObject)
            {
                bool pressed_down = SteamVR_Controller.Input((int)controller.controllerIndex).GetHairTriggerDown();
                if (pressed_down)
                {
    ...
    void FixedUpdate ()
    {
        ...
        if (last_grabber)
        {
            bool still_down = SteamVR_Controller.Input((int)last_grabber.GetComponent<SteamVR_TrackedController>().controllerIndex).GetTouch(Valve.VR.EVRButtonId.k_EButton_SteamVR_Trigger);
            if(still_down)
            {
